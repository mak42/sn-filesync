    ...
    inquirer
      ...
      .then(function (answers) {
        if (answers['conflict'] === 'resolve conflicts') {
          ...
          if (diff.getDiffCommand()) {
            // Run a diff on the head/local, using a given command
            fs.writeFileSync(diffFile, obj.records[0][map.field], "UTF-8");
            diff.runExternalDiff(log, diffFile, file);
            return;
          }
